...
/**
 * Fetch the top sell orders for an item and compute a sell value in platinum.
 *
 * @param {object} item        a WFM item object (needs `slug` and `tags`)
 * @param {object} opts
 * @param {number} opts.minPlat items whose value is below this are worth 0
 * @returns {Promise<{value:number, sellPrices:number[], subtype:string|null}>}
 */
export async function priceItem(item, { minPlat = 3 } = {}) {
  ...
  const avg = prices.length
    ...
    : 0;
  const value = avg < minPlat ? 0 : Math.round(avg * 100) / 100;

  return { value, sellPrices: prices, subtype: isRelic ? 'intact' : null };
}
