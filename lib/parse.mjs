  ...
  for (const [startId, endId, world] of BOUNTY_SECTIONS) {
    ...
    for (const row of rows) {
      ...
      if (th) {
        ...
        if (/Bounty\s*$/.test(t)) {
          // Drop the trailing "<World> Bounty" but keep subtype qualifiers like
          // "Ghoul" / "Arcana": "Level 40 - 50 Ghoul Bounty" -> "Level 40 - 50 Ghoul".
          const node = t
      ...
      if (tds.length >= 2 && tier) {
        ...
        // An item can appear in several stages; cap the per-run odds at 100%
        // ("chance to get at least one this run") rather than summing into an
        // expected count, which would overstate value and read as >100%.
        const existing = list.find((x) => x.item === item);
        if (existing) existing.chance = Math.min(1, existing.chance + chance);
