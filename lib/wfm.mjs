...
export async function priceItem(item) {
  ...
  // Mods and arcanes drop unranked, but maxed (rank 5) copies sell for far
  // more and dominate the cheapest orders when rank-0 sellers are thin. Price
  // the as-dropped state: rank 0. Relics drop intact, so price the intact tier.
  const ranked = typeof item.maxRank === 'number' && item.maxRank > 0;
  let query = '';
  if (isRelic) query = '?subtype=intact';
  else if (ranked) query = '?rank=0';
  const data = await getJson(`/orders/item/${item.slug}/top${query}`);

  let sell = data.sell || [];
  if (isRelic) sell = sell.filter((o) => o.subtype === 'intact');
  else if (ranked) sell = sell.filter((o) => o.rank === 0);

  // `/top` returns the cheapest online sell orders first (max 5).
  const lowest = sell.map((o) => o.platinum).slice(0, 5);
  // Discard troll/placeholder orders (e.g. a lone 999p in a thin market) that
  // would otherwise blow up the average: drop anything over 5x the cheapest.
  const prices = lowest.length ? lowest.filter((p) => p <= lowest[0] * 5) : [];
