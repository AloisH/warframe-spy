...
/**
 * Parse the "Derelict Vault" table (under "Dynamic Location Rewards:"): the
 * flat pool of corrupted mods opened with a Dragon Key. DE's export doesn't
 * split this by key type (Corrupt Charge / Extractor / Isolator / …) — it's
 * one table of 24 mods at equal odds (1/24 ≈ 4.17% each), so that's what we
 * expose: a single guaranteed reward drawn from the whole pool.
 *
 * @returns {Array<{item, chance}>}
 */
export function parseDerelictVault(html) {
  const start = html.indexOf('id="derelictvault"');
  if (start === -1) return [];
  const end = html.indexOf('blank-row', start);
  ...
  for (const row of rows) {
    ...
    if (tds.length === 2) {
      const item = tds[0];
      const pct = tds[1].match(/([\d.]+)\s*%/);
      out.push({ item, chance: pct ? parseFloat(pct[1]) / 100 : 0 });
    }
  }
  return out;
}
